...
def saveNewDevice():
    ...
    NofDongles= getDongleNumber()
    for A in range(NofDongles):
        adapter='hci'+str(A)
        for i in newList:
            poller = MiFloraPoller(i, BluepyBackend, retries=1, cache_timeout=60, adapter=adapter)
            print(poller.parameter_value(MI_CONDUCTIVITY))
            if (poller.parameter_value(MI_CONDUCTIVITY)>50 or poller.parameter_value(MI_MOISTURE)>40):
